...
public enum VehicleStatus { EmptyOff, EmptyOn, AI, Player, ChasePlayer }
...
public class AIVehicle : MonoBehaviour
{
   ...
   [HideInInspector]
    ...
    {
		if(isAutoCar==false){
			player = AIContoller.Instance.player;
		}
       
        currentFwdSpeed = forwardSpeed;

		if (vehicleStatus == VehicleStatus.ChasePlayer)
        {
			AIActive = true;
			oneWay = true;
		}
    ...
    void Update()
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            if (Physics.Raycast(raycastPoint.TransformPoint((Mathf.Repeat((float)i, 3) - 1), 0, 0), raycastPoint.TransformDirection((rotate * rayCastAngle), 0, (lentgh * rayCastLentgh)), out raycastHit, (lentgh * rayCastLentgh), layerMask.value, QueryTriggerInteraction.Collide))
            {
				//print (hitting+"hiiting");
                hitting = true;
                ...
                if (i == 3 || i == 0)
                {
                    ...
                    if (raycastHit.distance < 0.5f) { if (waitingTime == 0 && !RearGear) { waitingTime = Random.Range(0.5f,1.0f); if (horn)horn.Play(); } RearGear = true; }
                }
                //center front
                else if (i == 1)
                {
                    ...
                    if (raycastHit.distance < lentgh / 2.0f) { if (waitingTime == 0 && !RearGear) { waitingTime = Random.Range(0.5f, 1.0f); if (horn)horn.Play(); } RearGear = true; }
                }//center back
                else if (i == 4)
                {
        ...
        if (!AIBrake && !RearGear)
        {
           ...
           // print("RearGear......."+ RearGear);
            //print("AIBrake......." + AIBrake);
        }
        else
        {
          //  print("AIAccel.......");
            if (stopTimer == 3.0f) AIAccel = -currentFwdSpeed / 100.0f; 
            else AIAccel = 0.0f;
        }
        ...
        if (vehicleStatus != VehicleStatus.EmptyOn)
        {
            relativeTarget = transform.InverseTransformPoint(nextNode.position);
            ...
            else
            {
				...
				if (nodeComponenet)
                {
					if (((nodeComponenet.nodeState == "NextPoint" && myStatue == "PreviousPoint")
					    || (nodeComponenet.nodeState == "PreviousPoint" && myStatue == "NextPoint")) 
                        && nodeComponenet.trafficMode != TrafficMode.Go)
                    {
						trafficStop = true;
						AIBrake = true;
	...
	private WaysControl waysControl;

    Transform RandomWay(Transform node, int maxWays)
    {
        waysControl = currentNode.GetComponent<WaysControl>();
