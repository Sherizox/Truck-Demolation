...
public class VehicleControl : MonoBehaviour
{
    ...
    public bool funnyVehicle = false;
    ...
    private WheelComponent SetWheelComponent(Transform wheel, float maxSteer, bool drive, float pos_y)
    {
        ...
        if(funnyVehicle==false)
        //col.gameObject.AddComponent<WheelSkidmarks>();
        result.wheel = wheel;
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    private WheelFrictionCurve fc;
    public float waitToDrift = 3f;
    private float tempDriftWait = 0;
    private int driftCoins;
    private int TotalCoinsDirft;
    public bool isDriftCar = false;
    void Awake()
    {
		//status = GetComponent<StatusInGame> ();

        AIVScript = transform.GetComponent<AIVehicle>();

        myRigidbody = transform.GetComponent<Rigidbody>();
		BackLeftWheel = carWheels.wheels.backLeft;
		BackRightWheel= carWheels.wheels.backRight;
        if (carSounds.horn)
    ...
    void Update()
    {
        ...
        else if(AIVScript.vehicleStatus != VehicleStatus.ChasePlayer)
        {
            AIVScript.AIActive = false;
        }
	...
	private bool DriftParticlActive=false;
    private ParticleSystem pc;
    private Color tempAlphaChannel;
    private Color tempAlphaChannelSlideImage;
    private Color tempACMultiText;
    void FixedUpdate()
    {
        //if (GlobalConstant.isGameOver == true || GlobalConstant.isGameSuccess == true)
        //{
        //    steer = 0;
        //    speed = 0;
        ...
        if (AIVScript.vehicleStatus == VehicleStatus.Player) {
            //print (steer + "SteerAmmount");
            //TODO

            if ((steer > 0.5 || steer < -0.5) && speed > 20) {
                if (!carSounds.breakSound.isPlaying) {
                    carSounds.breakSound.Play();
                    if (funnyVehicle == false) {

                        //Particle2 = Instantiate(carParticles.brakeParticlePerfab, BackLeftWheel.position, Quaternion.identity) as GameObject;
                        //Particle2.transform.parent = transform;
                        //Particle3 = Instantiate(carParticles.brakeParticlePerfab, BackRightWheel.position, Quaternion.identity) as GameObject;
                        //Particle2.transform.parent =transform;
                ...
                DriftParticlActive = false;

                carSounds.breakSound.Stop();
                //print ("SoundStop");
            }
       ...
       // if (status.hp <= 0) {
        ...
        //}
        // speed of car
        speed = myRigidbody.velocity.magnitude * 1.7f;
        ...
        switch (AIVScript.vehicleStatus)
        {
            ...
            case VehicleStatus.AI:
                ...
                break;
            case VehicleStatus.ChasePlayer:
            ...
            case VehicleStatus.Player:
                ...
                //        //GlobalConstant.isPowerShifting = shift;
                //        if (powerShift > 0 && shift == false) {
                //            GlobalConstant.isPowerShifting = false;
                //        }
                ...
                //        if (GameControl.steerAmount > 0.1f) {
    ...
    : slip = Mathf.Lerp(slip, 0.01f, 0.02f);
            ...
            {
                //print ("powerShift...=" +powerShift);
                //if (speed > 50) {
               // GlobalConstant.isPowerShifting = true;
                //print ("isFuckingCalling");
                //}


                //print ("shift="+shift+" shiftmotor="+shifmotor+" Mathf.Abs(steer)="+Mathf.Abs(steer));
                if (powerShift == 0) {
                    shifmotor = false;
                   // GlobalConstant.isPowerShifting = false;
                }
                ...
                curTorque = 500f;//powerShift > 0 ? carSetting.shiftPower : carSetting.carPower;
            ...
            else
            {
                ...
                //print ("elssssssssssssssssssssss");
              ...
              //  carParticles.shiftParticle2.emissionRate = Mathf.Lerp(carParticles.shiftParticle2.emissionRate, 0, Time.deltaTime * 10.0f);
                if (speed <= 20 && AIVScript.vehicleStatus == VehicleStatus.Player) {
                   // GlobalConstant.isPowerShifting = false;
                    //print ("elssssssssssssssssssssss"+speed);
                }
            }
            ...
            if (col.GetGroundHit(out hit))
            {
                ...
                if (carParticles.brakeParticlePerfab)
                {
                    ...
                    if (funnyVehicle == false)
                    {
                        pc = Particle[currentWheel].GetComponent<ParticleSystem>();
                    }
        ...
        if (isDriftCar)
        {
            // dirft Count Code here
            if (fc.extremumSlip > 1.2 && speed>15)
            {
                tempAlphaChannel.a = 1;
